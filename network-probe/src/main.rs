...
fn annotate_event(
    ...
    let (local_ipv4, pod_name, pod_namespace, remote_ipv4) = get_pod_metadata(event, pod_store)?;
    let destination = get_event_destination(
        event,
        &pod_name,
        &pod_namespace,
...
fn get_pod_metadata(
    event: bpf_event,
    store: &Store<Pod>,
) -> Result<(Ipv4Addr, String, String, Ipv4Addr)> {
    ...
    let pod_namespace = pod.namespace().unwrap_or("unknown".to_string());
    let remote_ipv4 = Ipv4Addr::from(event.remote_ip4);
    Ok((local_ipv4, pod_name, pod_namespace, remote_ipv4))
}
...
async fn setup_tasks(
    ...
    let node_filter = Config::default();
    ...
    let node_reflector = reflector::reflector(
        node_writer,
        kube::runtime::watcher(node.clone(), node_filter),
    );
    ...
    let node_watch = node_reflector.applied_objects().for_each(|_| ready(()));
    ...
    let node_update_handle = task::spawn(async move {
        node_watch.await;
    });
    ...
    let enrichment_handle = task::spawn(async move {
        while let Some(event) = channel.recv().await {
            ...
            let process = match annotate_event(event, &pod_reader, &node_reader, &service_reader) {
                Ok(d) => d,
                Err(e) => {
            ...
            counter.add(
                ...
                &[
                    ...
                    KeyValue::new("local_port", process.raw_event.local_port.to_string()),
